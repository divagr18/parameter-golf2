...
RUN_ID="${RUN_ID:-sp8192_unigram_submission_8gpu_d464_nojpcr_s${SEED}_$(date +%Y%m%d_%H%M%S)}"
...
MODEL_DIM=464 NUM_LAYERS=9 NUM_HEADS=8 NUM_KV_HEADS=4 MLP_MULT=2 TIE_EMBEDDINGS=1 \
...
DISTILL_ENABLED=0 DISTILL_START_STEP=-1 DISTILL_START_FRAC=-1 DISTILL_START_WALLCLOCK_FRAC=-1 DISTILL_WEIGHT=0.08 DISTILL_TEMP=2.0 DISTILL_EMA_DECAY=0.999 \
JPCR_ENABLED=0 JPCR_HIDDEN=64 JPCR_PROJ_DIM=32 JPCR_WEIGHT=0.08 JPCR_BLEND_INIT=-2.0 JPCR_LR=0.02 JPCR_WARMUP_STEPS=100 \
