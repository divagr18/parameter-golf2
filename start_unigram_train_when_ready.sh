...
RUN_GROUP="${RUN_GROUP:-sp8192_unigram_parallel_$(date +%Y%m%d_%H%M%S)}"
RUN_ID="${RUN_ID:-sp8192_unigram_submission_8gpu_$(date +%Y%m%d_%H%M%S)}"
...
echo "[start] unigram ready, launching training RUN_ID=${RUN_ID}"
