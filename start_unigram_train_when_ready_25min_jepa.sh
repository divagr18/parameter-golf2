...
UNIGRAM_DATA="${UNIGRAM_DATA:-/workspace/parameter-golf/data/dual_unigram/datasets/fineweb10B_sp8192}"
UNIGRAM_TOK="${UNIGRAM_TOK:-/workspace/parameter-golf/data/tokenizers/fineweb_8192_unigram_20260422_225958.model}"
WAIT_SHARDS="${WAIT_SHARDS:-80}"
SEED="${1:-${SEED:-1337}}"

# 25 minutes = 1500 seconds. Keep ITERATIONS high so wallclock cap stops first.
MAX_WALLCLOCK_SECONDS="${MAX_WALLCLOCK_SECONDS:-1500}"
ITERATIONS="${ITERATIONS:-50000}"

# JPCR/JEPA activation timing (wallclock-based is robust when step time changes).
# Default: start at 60% of wallclock (~15 min into a 25 min run).
DISTILL_START_WALLCLOCK_FRAC="${DISTILL_START_WALLCLOCK_FRAC:-0.60}"
DISTILL_START_STEP="${DISTILL_START_STEP:--1}"
# Apply distill/JPCR every Nth step (avoids every-step JPCR overhead).
JPCR_APPLY_EVERY="${JPCR_APPLY_EVERY:-2}"

RUN_GROUP="${RUN_GROUP:-sp8192_unigram_25m_jepa_s${SEED}_$(date +%Y%m%d_%H%M%S)}"
RUN_ID="${RUN_ID:-sp8192_unigram_submission_8gpu_25m_jepa_s${SEED}_$(date +%Y%m%d_%H%M%S)}"

mkdir -p "logs/runs/${RUN_GROUP}" "artifacts/${RUN_GROUP}" ".cache/torchinductor" ".cache/triton"

# Reuse compile caches across runs to reduce compile latency.
...
# Safer compile mode for fixed-shape training + phase changes.
export TORCH_COMPILE_DYNAMIC="${TORCH_COMPILE_DYNAMIC:-none}"
# Safe fast path validated by smoke test (conditional params are zero-touched in loss).
...
echo "[start] launching 25m JEPA run RUN_ID=${RUN_ID} SEED=${SEED}"
