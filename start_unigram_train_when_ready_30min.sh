...
RUN_GROUP="${RUN_GROUP:-sp8192_unigram_parallel_30m_s${SEED}_$(date +%Y%m%d_%H%M%S)}"
RUN_ID="${RUN_ID:-sp8192_unigram_submission_8gpu_jepa_30m_s${SEED}_$(date +%Y%m%d_%H%M%S)}"
...
echo "[start] unigram ready, launching 30m training RUN_ID=${RUN_ID} SEED=${SEED}"
...
ITERATIONS=60000 MAX_WALLCLOCK_SECONDS=1800 \
...
DISTILL_ENABLED=1 DISTILL_START_STEP=-1 DISTILL_START_FRAC=-1 DISTILL_START_WALLCLOCK_FRAC=0.40 DISTILL_WEIGHT=0.08 DISTILL_TEMP=2.0 DISTILL_EMA_DECAY=0.999 \
