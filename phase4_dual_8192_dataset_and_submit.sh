...
if [ ! -d "${DATA_ROOT}/datasets/fineweb10B_sp8192" ]; then
...
if [ ! -d "${DATA_ROOT}/datasets/fineweb10B_sp8192_unigram" ]; then
  export VOCAB_SIZE=8192_unigram
  export EXISTING_TOKENIZER_MODEL="${TOKENIZERS_DIR}/fineweb_8192_unigram_20260422_225958.model"
  export OUTPUT_ROOT="${DATA_ROOT}"
  bash "${WORKSPACE_ROOT}/build_sp_dataset.sh"
  echo "✓ fineweb10B_sp8192_unigram dataset built successfully"
else
  echo "✓ fineweb10B_sp8192_unigram dataset already exists, skipping build"
fi
...
export VOCAB_SIZE=8192
export DATA_PATH="${DATA_ROOT}/datasets/fineweb10B_sp8192"
...
export VOCAB_SIZE=8192_unigram
export DATA_PATH="${DATA_ROOT}/datasets/fineweb10B_sp8192_unigram"
